...
import type { PaginatedFactoryList, TransferAbsence } from "@/types"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const apiUrl = new URL(`${process.env.API_URL}/api/tasks/transfer-absence/`)
    ...
    const TransferAbsences: TransferAbsence[] = await response.json()
    ...
    const updatedTransferAbsences = TransferAbsences.map((item: TransferAbsence) => ({
      ...
      factory_name_onsite: factoryMap[item.factory_code_onsite] || ""
    }))
