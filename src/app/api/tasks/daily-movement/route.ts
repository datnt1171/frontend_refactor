...
import type { PaginatedFactoryList, DailyMovement } from "@/types"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const apiUrl = new URL(`${process.env.API_URL}/api/tasks/daily-movement/`)
    ...
    const DailyMovements: DailyMovement[] = await response.json()
    ...
    const updatedDailyMovements = DailyMovements.map((item: DailyMovement) => ({
      ...item,
      factory_name: factoryMap[item.factory_code] || ""
    }))

    return NextResponse.json(updatedDailyMovements)
