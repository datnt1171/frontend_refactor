import {  } from '@/lib/api/server/api'
import { getUsers, getFactories, getRetailers, getTaskData, getTask } from '@/lib/api/server' // Add these imports
...
export default async function TaskDataPage({ params }: PageProps) {
  ...
  try {
    // Fetch task data and task details in parallel
    const [taskData, task] = await Promise.all([
      getTaskData(taskId, fieldId),
      getTask(taskId)
    ])

    // Find the specific field from task's process
